...
*/
  ...
  this.showMenuItems = function() {
    ...
    var fs = parent.data.menuFontSize;
    for (var i = 0; i < parent.itemsData.length; i++) {

      var camiryo = new p5(subItem);
      camiryo.initialize(parent.itemsData[i].name, i, fs,
        this.x[i], this.y[i],
        color(parent.data.homeRed, parent.data.homeGreen, parent.data.homeBlue), false);
      camiryo.setPosition(this.posx, this.posy);
      this.items.push(camiryo);
    }
